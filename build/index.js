...
const ApiControllers = AuthorizeNet.default.APIControllers;
const SDKConstants = AuthorizeNet.default.Constants;
...
async function getSettledBatchList(firstSettlementDate, lastSettlementDate) {
...
async function getTransactionList(batchId) {
...
async function getCustomerProfileTransactionList(customerProfileId) {
    ...
    try {
        ...
        switch (name) {
            case "get_settled_batch_list":
                result = await getSettledBatchList(args?.firstSettlementDate, args?.lastSettlementDate);
                break;
            case "get_transaction_list":
                result = await getTransactionList(args?.batchId);
                break;
            case "get_unsettled_transaction_list":
                result = await getUnsettledTransactionList();
                break;
            case "get_customer_profile_transaction_list":
                result = await getCustomerProfileTransactionList(args?.customerProfileId);
                break;
            case "get_transaction_details":
                result = await getTransactionDetails(args?.transactionId);
                break;
            case "get_batch_statistics":
                result = await getBatchStatistics(args?.batchId);
                break;
